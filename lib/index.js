...
export function apply(ctx) {
  ...
  ctx.on('tools/execute', async (exec, next) => {
    ...
    // Warm inflight deduplication + concurrency control (max 2 concurrent warms)
  ...
  const scheduleWarm = (task) => {
    ...
    // Deep mode: expand query before search, restore after
    const _origQuery = args.query
    let _expanded = false
    if (sfDeepMode && phase === 'search' && args.query && !callId.includes('-exp')) {
      const expanded = expandQuery(args.query)
      if (expanded !== args.query) {
        args.query = expanded
        _expanded = true
        log('deep mode: expanded query (original: ' + String(_origQuery).slice(0, 50) + ')')
      }
    }
    ...
    try {
      ...
      if (_expanded) args.query = _origQuery
      return result
    } catch (error) {
      if (_expanded) args.query = _origQuery
