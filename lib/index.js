...
// ─── URL canonicalization (B#3) ─────────────────────────────────────────
// Same-page URL variants (www prefix, trailing slash, default port, query
// param order) must share one cache key, otherwise redundant renders hit.
// Evidence: deepseek.com/en/index.html has www/non-www variants in store.db;
// 127.0.0.1:3080 has 11 entries split by trailing slash.
...
export function apply(ctx) {
  ...
  ctx.on('tools/execute', async (exec, next) => {
    ...
    if (exec.name === 'browser_open' && ckey && !args.fresh) {
      ...
      if (entry) {
        ...
        // Fire-and-forget the real navigation: active page must be ready for
        // follow-up click/type/scroll. The waterfall's `next` runs the real
        // browser_open (governor + approval chain intact); only the returned
        // value is discarded. A fresh render also backfills the memory cache
        // with htmlPath so later web_snapshot hits are complete.
        next().then((warmResult) => {
  ...
  ctx.effect(() => {
    ...
    log('SSE route /searchflow/events registered')
    return dispose
