...
`;
		...
		function mountIconFarm() {
			...
			react_dom_client.createRoot(farmHost).render(cx(IconFarm));
		}
		...
		function sfNotify() { if (window.__SF_DEBUG__) window.__SF_DEBUG__.task = sfTask; for (const fn of sfListeners) { try { fn(); } catch {} } }
		...
		function sfTouchTask(phase) {
			const now = Date.now();
			if (!sfTask || now - sfTask.lastEventAt > TASK_IDLE_MS) {
				sfTask = newTask();
			}
		...
		function sfHandleTaskEvent(evt) {
			...
			if (type === "tool:start") {
				sfTouchTask(isSearch ? "search" : "reading");
		...
		function SearchFlowCard(props) {
			...
			return cx("div", { "data-sf-card": true, "data-sf-variant": isTab ? "tab" : "card", style: isTab ? base : { ...base, width: 368, maxWidth: "92vw", overflow: "hidden", animation: reduced ? undefined : "sfExpand .22s cubic-bezier(.34,1.56,.64,1) both" } },
				...
				t.results.length > 0 && cx("div", { style: { padding: "2px 12px 8px", maxHeight: isTab ? undefined : 150, flex: isTab ? "1 1 auto" : undefined, minHeight: isTab ? 0 : undefined, overflowY: "auto", display: "flex", flexDirection: "column", gap: 4 } },
					t.results.slice(isTab ? -TAB_MAX_RESULTS : -CARD_MAX_RESULTS).map((it, i) => {
						const hue = sfHashHue(it.domain);
						const dup = (t.seenUrls.get(it.url) || 0) >= 2;
						return cx("a", { key: it.url + ":" + i, href: it.url, target: "_blank", rel: "noreferrer", title: it.url, style: { display: "flex", alignItems: "center", gap: 9, padding: "6px 8px", borderRadius: 9, background: "rgba(255,255,255,.04)", border: "1px solid rgba(255,255,255,.055)", textDecoration: "none", color: "inherit", transition: "background .18s ease, border-color .18s ease, transform .18s ease", animation: reduced ? undefined : "sfItemIn .3s ease both", animationDelay: reduced ? undefined : i * 70 + "ms" } },
		...
		function mountStandaloneCard() {
			...
			sfCardRoot = react_dom_client.createRoot(sfCardMount);
			sfCardRoot.render(cx(SearchFlowCard, { variant: "card" }));
		}
		function teardownStandaloneCard() {
			if (sfCardRoot) { try { sfCardRoot.unmount(); } catch {} sfCardRoot = null; }
